...
export function setPricingCachePath(newPath) {
    if (newPath.includes('..') || newPath.startsWith('/')) {
        throw new Error('Invalid cache path');
    }
    CACHE_PATH = newPath;
}
export function loadPricingCache() {
...
export function savePricingCache(cache) {
...
export function getModelPricing(cache, model) {
