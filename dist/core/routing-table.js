...
export function generateRoutingTable(benchmarks, pricing, availableModels, config) {
    const skills = ['brainstorming', 'writing-plans', 'executing-plans', 'reviewing'];
    const domains = ['gui', 'backend', 'security', 'database', 'general'];
    const risks = ['critical', 'high', 'medium', 'low', 'micro'];
    const rules = [];
    for (const skill of skills) {
        for (const domain of domains) {
            ...
            for (const model of availableModels) {
                ...
                if (benchmarkScore === null || pricingData === null)
                    continue;
                if (rank !== null && rank > config.minBenchmarkRank)
                    continue;
                if (pricingData.avgCostPer1k > config.costCeilingPer1kTokens)
                    continue;
                const scorePerDollar = benchmarkScore / pricingData.avgCostPer1k;
                if (isNaN(scorePerDollar))
                    continue;
...
export function mapSkillDomainToCategory(skill, domain) {
    const mappings = {
